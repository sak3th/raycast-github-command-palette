...
import { useSearchIssuesInRepo, useSearchPullsInRepo } from "../../service/issue";
import { getIssueIcon, getAccessories } from "./common";
import { toIssue } from "./model";

export function RepoPulls({ repo }: { repo: Repo }) {
  const [searchQuery, setSearchQuery] = useState("");
  const [cache, setCache] = useState<Issue[]>([]);
  const { isLoading: searchIsLoading, data: searchData, error: searchError } = useSearchPullsInRepo(searchQuery, repo);
